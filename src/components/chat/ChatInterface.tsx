...
import { Send, Mic, MicOff, Trash2, Sparkles, AlertCircle } from 'lucide-react';
...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({
  ...
  // Production: VITE_API_URL from .env.production
  const API_BASE_URL =
  ...
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      recognition.onerror = (err: any) => {
        ...
        setVoiceError(
          'Microphone input error. Please check browser permissions.'
        );
  ...
  const toggleVoiceInput = () => {
    if (!recognitionRef.current) {
      setVoiceError(
        'Voice input is not supported in this browser. Try Chrome/Edge.'
      );
  ...
  const handleSendMessage = async (textToSend?: string) => {
    ...
    try {
      // IMPORTANT:
      // Instead of /api/chat/stream,
      // we now use the deployed backend URL.
